...
Build artifacts and verification files are available in [GitHub Releases](https://github.com/KBin-001/PayDance/releases).
...
- **V1.0 release**: aligned desktop and web version labels; GitHub, updater, and download links now target KBin-001/PayDance, and the website footer includes a contact email.

- **Live effective hourly rate**: a new line under "Today's Earnings" shows the current rate — today's earnings divided by the hours actually worked today. During normal hours it is exactly the rate your settings work out to (¥375 for an 8-hour day → ¥46.88/h); once you work past your end time the money stops growing while the hours keep coming, so the number is diluted honestly — the same ¥375 day plus two unpaid overtime hours reads ¥37.50/h. Overtime counts for up to four hours, after which the number freezes instead of sliding towards zero while the window stays open. "Worked" now keeps growing past your end time too, while today's earnings and progress stay at the full day and 100% (this release still pays no overtime). The line follows your currency symbol and is fully bilingual: what you see during overtime is the rate falling, not the amount rising.
- **Alternating work weeks**: a new switch under "Workdays" in Settings, for people whose every other week has one more workday. Turning it on makes the workday picker describe the "small" week (the one with fewer workdays) and reveals "Extra workdays in big weeks" (Saturday by default, limited to the days that week leaves free). A config that has never been aligned takes the week you switch it on in as a big week; one that already has an alignment keeps it, so switching the toggle off and on again never resets it. The two weeks alternate from there: the dashboard status, today's earnings and the night-shift check all follow. If a week looks wrong, "This week is" next to it realigns on the spot. Turning the toggle on without picking an extra workday, picking one the small week already works, or carrying an unreadable start week shows "Needs Setup" and names the setting at fault; none of it is checked while the toggle is off. Under monthly pay, turning alternating weeks on shows a hint next to "Work days per month": the monthly average rises from about 21.7 workdays to about 23.9, so around 24 is usually right — the app only hints and never changes that number itself. The switch is off by default, so an existing install keeps its old numbers.
...
- **Maintenance chain**: CI runs the whole unit-test suite on every run, so Rust, script or workflow changes can no longer skip the tests that cover them; a failed Release removes its tag so the fix can ship under the same version; `npm run usage:snapshot` records Release asset download counts in a CSV on the maintainer's machine as a telemetry-free usage signal; the dependency bot switched from Renovate to Dependabot (since 2026-08-08).
